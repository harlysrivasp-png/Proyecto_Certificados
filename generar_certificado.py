...
def generar_certificado(
    ...
    # Logo UCEVA
    c.drawImage(ImageReader(logo), 3*cm, height - 18.5*cm, width=4*cm, preserveAspectRatio=True, mask='auto')
    ...
    c.line(width/5 - firma_ancho/2, y_firma + 0.5*cm, width/4 + firma_ancho/2, y_firma + 0.5*cm)
    ...
    # Línea horizontal bajo la firma
    c.line(3*width/4 - firma_ancho/2, y_firma + 0.5*cm, 3*width/6 + firma_ancho/2, y_firma + 0.5*cm)
    c.setFont("Helvetica-Bold", 12)
